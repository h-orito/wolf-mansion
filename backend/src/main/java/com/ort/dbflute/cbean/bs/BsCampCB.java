...
public class BsCampCB extends AbstractConditionBean {
    ...
    //                                                                           Attribute
    //                                                                           =========
    protected CampCQ _conditionQuery;
    ...
    public CampCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected CampCQ doGetConditionQuery() {
    ...
    protected CampCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected CampCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        CampCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected CampCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new CampCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<CampCB> unionCBLambda) {
        final CampCB cb = new CampCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final CampCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<CampCB> unionCBLambda) {
        final CampCB cb = new CampCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final CampCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
    ...
    public static class HpSpecification extends HpAbstractSpecification<CampCQ> {
        public HpSpecification(ConditionBean baseCB, HpSpQyCall<CampCQ> qyCall
         ...
         */
        public HpSDRFunction<CampAllocationCB, CampCQ> derivedCampAllocation() {
            assertDerived("campAllocationList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<CampAllocationCB> sq, CampCQ cq, String al, DerivedReferrerOption op)
         ...
         *     skillCB.specify().<span style="color: #CC4747">column...</span> <span style="color: #3F7E5E">// derived column by function</span>
         *     skillCB.query().set... <span style="color: #3F7E5E">// referrer condition</span>
         * }, Skill.<span style="color: #CC4747">ALIAS_foo...</span>);
         * </pre>
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<SkillCB, CampCQ> derivedSkill() {
         ...
         */
        public HpSDRFunction<CampCB, CampCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<CampCB> sq, CampCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public CampCB dreamCruiseCB() {
        CampCB cb = new CampCB();
        cb.xsetupForDreamCruise((CampCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<CampCB> columnQuery(final SpecifyQuery<CampCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected CampCB xcreateColumnQueryCB() {
        CampCB cb = new CampCB();
        cb.xsetupForColumnQuery((CampCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<CampCB> orCBLambda) {
        xorSQ((CampCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<CampCB> andCBLambda) {
        xorSQAP((CampCB)this, andCBLambda);
    }
    ...
    protected String xgetConditionBeanClassNameInternally() { return CampCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return CampCQ.class.getName(); }
