...
data class Skills(
    ...
    companion object {
        ...
        // 転生で割り当て可能な役職
        fun revivables(): Skills =
            Skills(
                list = all().filterNotSomeone().list.filter { it.isRevivable() },
            )

        // 役職希望で選択できる役職
        fun requestables(): Skills =
            Skills(
                list = all().list.filter { it.isRequestable() },
            )

        // 編成に含めることができる役職
        fun organizables(): Skills =
            Skills(
                list = all().filterNotSomeone().list.filter { it.isRequestable() },
            )

        fun getSkillListStr(): String = organizables().list.joinToString(separator = " / ") { "${it.name}:${it.shortName}" }
    }
