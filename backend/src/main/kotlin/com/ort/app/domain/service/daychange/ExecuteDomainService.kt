...
class ExecuteDomainService(
    ...
    fun execute(daychange: Daychange): Daychange {
        ...
        val executeCount =
            village.participants
                .filterAlive()
                .filterBySkill(CDef.Skill.執行人.toModel())
                .list.size + 1
        // 投票を隠すか
        val existsBlackbox =
            daychange.abilities
                ...
                .list
                .any { village.participants.chara(it.charaId).isAlive() }
        // 革命中か
        val existsRevolution =
            daychange.abilities
        ...
        votes
            .filter {
                village.participants
                    .chara(it.targetCharaId)
                    .skill!!
                    .toCdef() == CDef.Skill.王族
            }.forEach {
        ...
        executedParticipants
            .filter { it.skill!!.toCdef() == CDef.Skill.帝狼 }
            .forEach { executed ->
        ...
        val tawawaCharaIds =
            daychange.abilities
                .filterByDay(village.latestDay() - 1)
                .filterByType(CDef.AbilityType.ナマ足.toModel())
                .list
                .map {
                    it.charaId
                }
        ...
        votes
            .filter { tawawaCharaIds.contains(it.targetCharaId) }
    ...
    private fun calculateVotePersonCount(
        village: Village,
        votes: List<Vote>,
    ): Map<VillageParticipant, Int> =
        votes
    ...
    private fun decideExecutedParticipant(
        ...
        val votedCount =
            if (existsRevolution) {
                votedMap
                    .filterNot { (_, count) -> count == 0 }
                    .map { (_, count) -> count }
                    .minOrNull()
            } else {
                votedMap.map { (_, count) -> count }.maxOrNull()
            }
        ...
        return if (nonLuckymanCandidates.isNotEmpty()) {
            nonLuckymanCandidates.shuffled().first()
        } else {
            candidates.shuffled().first()
        }
    }

    private fun createEachVoteMessage(
        village: Village,
        votes: List<Vote>,
        ...
        val messageType =
            if (village.setting.rule.isOpenVote && !existsBlackbox) {
