...
data class Skill(
    ...
    fun hasInvestigateAbility(): Boolean = toCdef() == CDef.Skill.探偵 || toCdef() == CDef.Skill.監視者 || toCdef() == CDef.Skill.闇探偵
    ...
    fun isViewableDivineMessage(): Boolean = toCdef() != CDef.Skill.賢者 && (hasDivineAbility() || hasDeadDivineAbility())

    fun isViewableWiseMessage(): Boolean = toCdef() == CDef.Skill.賢者

    fun isViewableInvestigateMessage(): Boolean = toCdef() == CDef.Skill.探偵 || toCdef() == CDef.Skill.監視者 || toCdef() == CDef.Skill.闇探偵
    ...
    companion object {
        private val skillToAbility =
            CDef.Skill.listOfHasAttackAbility().associateWith {
                ...
                mapOf(
                    ...
                    CDef.Skill.浮気者 to AbilityType(CDef.AbilityType.浮気),
        ...
        private val shortNameToSkill =
            Skills.all().filterNotSomeone().list.associate {
                it.shortName to it.toCdef()
            }

        private val villagerSkills =
            Skills
                .all()
                .filterNotSomeone()
                .filterByCamp(CDef.Camp.村人陣営)
                .list
        private val wolfSkills =
            Skills
                .all()
                .filterNotSomeone()
                .filterByCamp(CDef.Camp.人狼陣営)
                .list
        private val foxSkills =
            Skills
                .all()
                .filterNotSomeone()
                .filterByCamp(CDef.Camp.狐陣営)
                .list
        private val loverSkills =
            Skills
                .all()
                .filterNotSomeone()
                .filterByCamp(CDef.Camp.恋人陣営)
                .list
        private val criminalSkills =
            Skills
                .all()
                .filterNotSomeone()
                .filterByCamp(CDef.Camp.愉快犯陣営)
                .list
        private val jingaiSkills = wolfSkills + foxSkills + criminalSkills
        private val sayableSkills =
            Skills.all().filterNotSomeone().list.filter {
                it.isSayableWerewolfSay() ||
                    it.isSayableSympathizeSay() ||
                    it.isSayableTelepathy() ||
                    it.toCdef() == CDef.Skill.恋人 ||
                    it.toCdef() == CDef.Skill.同棲者
            }
        private val notSayableSkills =
            Skills
                ...
                .filterNotSomeone()
                .list
                .filter { !sayableSkills.map { it.code }.contains(it.code) }
        private val footstepSkills =
            Skills.all().list.filter {
                it.hasAttackAbility() ||
        ...
        // 毎日使用可能
        val availableEverydaySkills =
            CDef.Skill.listOfHasAttackAbility() +
        ...
        // 対象指定＆足音発生
        val targetingAndFootstepSkills =
            CDef.Skill.listOfHasAttackAbility() +
                CDef.Skill.listOfHasDivineAbility() +
                listOf(
                    CDef.Skill.覚者,
