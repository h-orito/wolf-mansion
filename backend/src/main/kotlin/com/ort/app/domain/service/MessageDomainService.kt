...
class MessageDomainService(
    ...
    fun createSkillRequestMessage(participant: VillageParticipant): Message =
        Message.ofSystemMessage(
            ...
            message =
                "${participant.name()}が第一希望役職を${participant.requestSkill!!.first.name}に、" +
    ...
    fun createOrganizationMessage(
        ...
        val type =
            if (village.setting.rule.isRandomOrganization) {
                MessageType(CDef.MessageType.非公開システムメッセージ)
            } else {
                MessageType(
    ...
    fun createSkillAssignedMessage(village: Village): Message {
        val message =
            village.participants.list.joinToString(separator = "\n") {
    ...
    fun createRoomAssignedMessage(village: Village): Message {
        val message =
            village.participants.sortedByRoomNumber().list.joinToString(
    ...
    fun createRoomReAssignedMessage(
        village: Village,
    ...
    // 個人ごとでない能力行使結果
    fun createPublicAbilityMessage(
        village: Village,
        text: String,
        messageType: MessageType,
    ): Message =
    ...
    fun createAttackMessage(
        ...
        val t =
            if (attacker.skill?.toCdef() == CDef.Skill.バー狼) {
    ...
    fun createSayMessages(
        ...
        val transformation =
            if (village.status.isSettled()) {
                MessageTransformation() // エピでは変換しない
            } else {
                MessageTransformation(
                    ...
                    translate =
                        messageContent.type.toCdef() == CDef.MessageType.通常発言 &&
                            abilities.isTargetedYesterday(village, myself, CDef.AbilityType.翻訳.toModel()),
                    barlow =
                        messageContent.type.toCdef() == CDef.MessageType.人狼の囁き &&
    ...
    // [[allwho]]の変換
    private fun replaceAllwhoMessage(
        mes: String,
        participants: VillageParticipants,
    ): String {
    ...
    // [[gwho]]の変換
    private fun replaceGwhoMessage(
        mes: String,
        participants: VillageParticipants,
    ): String {
    ...
    // ユーザ定義キーワードの変換
    private fun replaceUserDefinedRandomMessage(
        mes: String,
    ...
    fun isViewable(
        ...
        if (messageType == CDef.MessageType.村建て発言) {
            true
        } else {
