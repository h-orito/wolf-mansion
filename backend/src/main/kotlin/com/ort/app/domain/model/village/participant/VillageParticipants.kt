...
data class VillageParticipants(
    ...
    fun findByRoomNumber(
        roomNumber: Int,
        day: Int,
    ): VillageParticipant? = list.find { it.roomNumberWhen(day) == roomNumber }
    ...
    fun filterExistsDeadHistory(
        day: Int,
        reason: DeadReason,
    ): VillageParticipants = copy(list = list.filter { it.dead.histories.deadWhen(day, reason) })
    ...
    fun filterExistsReviveHistory(day: Int): VillageParticipants = copy(list = list.filter { it.dead.histories.reviveWhen(day) })

    fun filterSuddenly(): VillageParticipants = copy(list = list.filter { it.dead.isSuddenlyDead() })

    fun filterNotDummy(dummyParticipant: VillageParticipant) = copy(list = list.filterNot { it.id == dummyParticipant.id })

    fun filterBySkill(skill: Skill): VillageParticipants = copy(list = list.filter { it.skill?.toCdef() == skill.toCdef() })
    ...
    fun sortedByRoomNumber(): VillageParticipants =
        this.copy(
            list =
                list.sortedWith(
                    compareBy<VillageParticipant> { it.isSpectator }
    ...
    fun assignSkill(
        ...
        skill: Skill,
        day: Int,
    ): VillageParticipants = transformParticipant(id) { it.assignSkill(skill, day) }

    fun assignRoom(
        id: Int,
        roomNumber: Int,
        day: Int,
    ): VillageParticipants = transformParticipant(id) { it.assignRoom(roomNumber, day) }

    // 死亡・復活
    fun suddenlyDeadh(
        id: Int,
        day: Int,
    ): VillageParticipants = transformParticipant(id) { it.suddenlyDeath(day) }

    fun execute(
        id: Int,
        day: Int,
    ): VillageParticipants = transformParticipant(id) { it.execute(day) }

    fun divineKill(
        id: Int,
        day: Int,
    ): VillageParticipants = transformParticipant(id) { it.divineKill(day) }

    fun attacked(
        id: Int,
        day: Int,
    ): VillageParticipants = transformParticipant(id) { it.attacked(day) }

    fun trapKill(
        id: Int,
        day: Int,
    ): VillageParticipants = transformParticipant(id) { it.trapKill(day) }

    fun bombKill(
        id: Int,
        day: Int,
    ): VillageParticipants = transformParticipant(id) { it.bombKill(day) }

    fun zakoKilled(
        id: Int,
        day: Int,
    ): VillageParticipants = transformParticipant(id) { it.zakoKilled(day) }

    fun suicide(
        id: Int,
        day: Int,
    ): VillageParticipants = transformParticipant(id) { it.suicide(day) }

    fun forceReincarnation(
        id: Int,
        day: Int,
        skill: Skill,
    ): VillageParticipants = transformParticipant(id) { it.forceReincarnation(day, skill) }

    fun revive(
        id: Int,
        day: Int,
    ): VillageParticipants = transformParticipant(id) { it.revive(day) }
    ...
    fun foxPossession(
        ...
        fromId: Int,
        toId: Int,
    ): VillageParticipants = transformParticipant(toId) { it.foxPossessioned(village, fromId) }

    fun insane(
        village: Village,
        fromId: Int,
        toId: Int,
    ): VillageParticipants = transformParticipant(toId) { it.insaned(village, fromId) }

    fun persuade(
        village: Village,
        fromId: Int,
        toId: Int,
    ): VillageParticipants = transformParticipant(toId) { it.persuaded(village, fromId) }
    ...
    fun addLover(
        ...
        to: VillageParticipant,
    ): VillageParticipants = transformParticipant(from.id) { it.addLover(to.id) }

    fun stalking(
        fromId: Int,
        toId: Int,
    ): VillageParticipants = transformParticipant(fromId) { it.stalking(toId) }

    fun cheatLove(
        village: Village,
        fromId: Int,
        toId: Int,
    ): VillageParticipants = transformParticipant(fromId) { it.cheatLove(village, toId) }

    fun seduce(
        fromId: Int,
        toId: Int,
    ): VillageParticipants = transformParticipant(toId) { it.seduced(fromId) }

    fun insurance(
        fromId: Int,
        toId: Int,
    ): VillageParticipants = transformParticipant(toId) { it.insurance(fromId) }

    fun breakup(
        id: Int,
        village: Village,
    ): VillageParticipants = transformParticipant(id) { it.breakup(village) }

    fun stealLove(
        id: Int,
        stealerId: Int,
        village: Village,
    ): VillageParticipants = transformParticipant(id) { it.stealLove(stealerId, village) }
    ...
    fun disrespect(
        fromId: Int,
        toId: Int,
    ): VillageParticipants = transformParticipant(toId) { it.disrespect(fromId) }
    ...
    private fun transformParticipant(
        id: Int,
        transform: (VillageParticipant) -> (VillageParticipant),
