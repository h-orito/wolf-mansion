...
class RoomDomainService(
    ...
    fun detectSilentRoomNumbers(
        ...
        return village.participants
            ...
            .filter { participant ->
                ...
                // 足音なしで襲撃できるか、
                if (candidates.size == 1 && candidates.first() == "なし") {
                    true
                } // 生存者がいないため無音になる
                else {
    ...
    private fun isLeftSide(
        targetRoomNumber: Int,
        width: Int,
    ): Boolean = targetRoomNumber % width == 1

    private fun isRightSide(
        targetRoomNumber: Int,
        width: Int,
    ): Boolean = targetRoomNumber % width == 0
