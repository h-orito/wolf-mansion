...
class DivineDomainService(
    ...
    fun divine(daychange: Daychange): Daychange {
        ...
        val existsCloud =
            daychange.abilities
                ...
                .filterByType(AbilityType(CDef.AbilityType.曇天))
                .list
                .isNotEmpty()
    ...
    private fun createDivineMessage(
        ...
        val text: String =
            when (myself.skill!!.toCdef()) {
                ...
                CDef.Skill.占星術師 -> {
                    if (existsCloud) {
                        createAstrologerFailedMessageText(myself, target)
                    } else {
                        createAstrologerDivineMessageText(village, myself, target)
                    }
                }
    ...
    private fun createSeerDivineMessageText(
        myself: VillageParticipant,
        target: VillageParticipant,
    ): String {
    ...
    private fun createWiseDivineMessageText(
        myself: VillageParticipant,
        target: VillageParticipant,
    ...
    private fun createAstrologerFailedMessageText(
        myself: VillageParticipant,
        target: VillageParticipant,
    ): String = "${myself.name()}は、${target.name()}のあたりを占おうとしたが、本日は曇天のため占星術を行えなかった。"

    private fun createFlowerDivineMessageText(
        myself: VillageParticipant,
        target: VillageParticipant,
    ): String {
    ...
    private fun divineKillIfNeeded(
        village: Village,
        myself: VillageParticipant,
        target: VillageParticipant,
    ): Village =
