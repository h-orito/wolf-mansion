...
class CharaDataSource(
    ...
    override fun findCharachips(
        ids: List<Int>,
        isOriginal: Boolean,
    ): Charachips {
        if (ids.isEmpty()) return Charachips(emptyList())
        return if (isOriginal) {
            mapOriginalCharachips(selectOriginalCharaGroups(ids))
        } else {
            mapCharachips(selectCharaGroups(ids))
        }
    }

    override fun findCharachip(
        id: Int,
        isOriginal: Boolean,
    ): Charachip? =
        if (isOriginal) {
            selectOriginalCharaGroup(id)?.let { mapOriginalCharachip(it) }
        } else {
            selectCharaGroup(id)?.let { mapCharachip(it) }
        }

    override fun findCharas(
        charachipId: Int,
        isOriginal: Boolean,
    ): Charas =
        if (isOriginal) {
            mapOriginalCharas(selectOriginalCharas(charachipId))
        } else {
            mapCharas(selectCharas(charachipId))
        }

    override fun findCharasByCharaIdList(
        idList: List<Int>,
        isOriginal: Boolean,
    ): Charas {
        if (idList.isEmpty()) return Charas(emptyList())
        return if (isOriginal) {
            mapOriginalCharas(selectOriginalCharas(idList))
        } else {
            mapCharas(selectCharas(idList))
        }
    }

    override fun findChara(
        id: Int,
        isOriginal: Boolean,
    ): Chara? =
        if (isOriginal) {
            selectOriginalChara(id)?.let { mapOriginalChara(it) }
        } else {
            selectChara(id)?.let { mapChara(it) }
        }
    ...
    private fun selectOriginalCharaGroups(ids: List<Int>): ListResultBean<OriginalCharaGroup> {
        val groupList =
            originalCharaGroupBhv.selectList {
    ...
    private fun selectOriginalCharaGroup(id: Int): OriginalCharaGroup? {
        val optGroup =
            originalCharaGroupBhv.selectEntity {
    ...
    private fun insertOriginalChara(
        charachipId: Int,
        name: String,
        shortName: String,
    ): Int {
    ...
    private fun insertOriginalCharaImage(
        charaId: Int,
        faceTypeName: String,
    ): Int {
    ...
    private fun updateOriginalCharaImageUrl(
        charaImageId: Int,
        url: String,
    ) {
        val ci = OriginalCharaImage()
    ...
    private fun updateOriginalCharaImageName(
        charaImageId: Int,
        name: String,
        isDisplay: Boolean,
    ) {
    ...
    private fun mapCharachip(group: CharaGroup): Charachip =
        Charachip(
            ...
            designer =
                group.designer.get().let { designer ->
                    Designer(
    ...
    private fun mapChara(chara: DbChara): Chara =
        Chara(
            ...
            images =
                CharaImages(
                    list =
                        chara.charaImageList.map { charaImage ->
                            CharaImage(
    ...
    private fun mapOriginalChara(chara: OriginalChara): Chara =
        Chara(
            ...
            images =
                CharaImages(
                    list =
                        chara.originalCharaImageList.map { charaImage ->
                            CharaImage(
                                faceType =
                                    FaceType(
