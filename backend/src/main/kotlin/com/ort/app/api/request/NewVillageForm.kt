...
data class NewVillageForm(
    ...
    private fun initializeSayRestrictList(): List<SkillSayRestrictForm> =
        Skills.all().filterNotSomeone().list.map {
    ...
    private fun initializeRpSayRestrictList(): List<MessageTypeSayRestrictForm> =
        listOf(CDef.MessageType.アクション).map {
            MessageTypeSayRestrictForm(it)
        }
    ...
    fun override(village: Village) {
        ...
        campAllocationList =
            campAllocationList!!.map { campAllocation ->
                ...
                campAllocation.copy(
                    ...
                    skillAllocation =
                        campAllocation.skillAllocation!!.map { skillAllocation ->
        ...
        sayRestrictList =
            sayRestrictList!!.map { formRestrict ->
                val restrict =
                    village.setting.sayRestriction.normalSayRestriction
                        .find { it.skill.code == formRestrict.skillCode }
                formRestrict.copy(
        ...
        skillSayRestrictList =
            skillSayRestrictList!!.map { formRestrict ->
                val restrict =
                    village.setting.sayRestriction.skillSayRestriction
                        .find { it.messageType.code == formRestrict.messageTypeCode }
                formRestrict.copy(
                    restrict = restrict != null,
                    count = restrict?.count ?: 20,
                    length = restrict?.length ?: 400,
        ...
        rpSayRestrictList =
            rpSayRestrictList!!.map { formRestrict ->
                val restrict =
                    village.setting.sayRestriction.skillSayRestriction
                        .find { it.messageType.code == formRestrict.messageTypeCode }
                formRestrict.copy(
                    ...
                    sayRestriction =
                        SayRestriction(
                            normalSayRestriction =
                                sayRestrictList!!.filter { it.restrict == true }.map {
