...
data class VillageGetMessageListForm(
    ...
    fun toMessageQuery(village: Village) =
        MessageQuery(
            village = village,
    ...
    private fun mappingToTypes(): List<MessageType> = types?.split(",")?.flatMap { typeMap.getOrDefault(it, emptyList()) } ?: emptyList()
    ...
    companion object {
        val typeMap =
            mapOf(
                ...
                CDef.MessageType.公開システムメッセージ.code() to
                    listOf(
                        CDef.MessageType.公開システムメッセージ.toModel(),
                        CDef.MessageType.参加者一覧.toModel(),
                    ),
                CDef.MessageType.非公開システムメッセージ.code() to
                    listOf(
