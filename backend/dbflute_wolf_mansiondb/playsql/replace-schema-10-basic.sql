...
(
    REFRESH_TOKEN_ID  INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'リフレッシュトークンID',
    PLAYER_ID         INT UNSIGNED NOT NULL COMMENT 'プレイヤーID',
    TOKEN_HASH        CHAR(64)     NOT NULL UNIQUE COMMENT 'リフレッシュトークンのSHA-256ハッシュ(hex)',
    ISSUED_DATETIME   DATETIME     NOT NULL COMMENT '発行日時',
    EXPIRES_DATETIME  DATETIME     NOT NULL COMMENT '有効期限',
    USED_DATETIME     DATETIME     COMMENT '使用(ローテーション)日時。使用済みは再利用不可',
    REVOKED_DATETIME  DATETIME     COMMENT '失効日時。ログアウトや漏洩疑い等で失効',
    REGISTER_DATETIME DATETIME     NOT NULL COMMENT '登録日時',
    REGISTER_TRACE    VARCHAR(64)  NOT NULL COMMENT '登録トレース',
    UPDATE_DATETIME   DATETIME     NOT NULL COMMENT '更新日時',
    UPDATE_TRACE      VARCHAR(64)  NOT NULL COMMENT '更新トレース',
    PRIMARY KEY (REFRESH_TOKEN_ID),
    -- 期限切れトークンの一括掃除 (EXPIRES_DATETIME < now()) でフルスキャンを避けるための索引
    INDEX IX_REFRESH_TOKEN_EXPIRES (EXPIRES_DATETIME)
) COMMENT = 'リフレッシュトークン';


-- ログイン失敗試行 (ブルートフォース対策のレート制限用)。
-- LOGIN_NAME は存在しないアカウントへの試行もありうるため PLAYER への FK は張らない。
-- 時間窓内の失敗数を 2 軸 (アカウント / IP) で数えるため各軸 + ATTEMPT_DATETIME に索引を張る。
CREATE TABLE LOGIN_FAILURE
(
    LOGIN_FAILURE_ID  INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'ログイン失敗ID',
    LOGIN_NAME        VARCHAR(60)  NOT NULL COMMENT '試行されたログインID(存在しないこともある)',
    IP_ADDRESS        VARCHAR(64)  NOT NULL COMMENT 'クライアントIP(CF-Connecting-IP優先)',
    ATTEMPT_DATETIME  DATETIME     NOT NULL COMMENT '失敗試行日時',
    REGISTER_DATETIME DATETIME     NOT NULL COMMENT '登録日時',
    REGISTER_TRACE    VARCHAR(64)  NOT NULL COMMENT '登録トレース',
    UPDATE_DATETIME   DATETIME     NOT NULL COMMENT '更新日時',
    UPDATE_TRACE      VARCHAR(64)  NOT NULL COMMENT '更新トレース',
    PRIMARY KEY (LOGIN_FAILURE_ID),
    INDEX IX_LOGIN_FAILURE_NAME (LOGIN_NAME, ATTEMPT_DATETIME),
    INDEX IX_LOGIN_FAILURE_IP (IP_ADDRESS, ATTEMPT_DATETIME)
) COMMENT = 'ログイン失敗試行';


-- 推理補助 (analyzer) のプレイヤー別メモ。
-- 内容はクライアントが編集する構造化メモ一式で、サーバは中身を解釈しないため JSON のまま保持する。
CREATE TABLE ANALYZER_MEMO
(
    ANALYZER_MEMO_ID  INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '推理補助メモID',
    PLAYER_ID         INT UNSIGNED NOT NULL COMMENT 'プレイヤーID',
    VILLAGE_ID        INT UNSIGNED NOT NULL COMMENT '村ID',
    MEMO_JSON         MEDIUMTEXT   NOT NULL COMMENT '推理補助メモの内容(JSON)',
    REGISTER_DATETIME DATETIME     NOT NULL COMMENT '登録日時',
    REGISTER_TRACE    VARCHAR(64)  NOT NULL COMMENT '登録トレース',
    UPDATE_DATETIME   DATETIME     NOT NULL COMMENT '更新日時',
    UPDATE_TRACE      VARCHAR(64)  NOT NULL COMMENT '更新トレース',
    PRIMARY KEY (ANALYZER_MEMO_ID),
    UNIQUE UQ_ANALYZER_MEMO (PLAYER_ID, VILLAGE_ID)
) COMMENT = '推理補助メモ';


-- プレイヤーごとのお気に入りキャラ。本人のみ閲覧/更新可能 (参加検討中のキャラを他者に見せないため)。
-- オリジナルキャラチップは入村時に画像アップロードでキャラ選択自体が無いため、標準キャラ (CHARA) のみ対象。
CREATE TABLE PLAYER_FAVORITE_CHARA
(
    PLAYER_FAVORITE_CHARA_ID INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'お気に入りキャラID',
    PLAYER_ID                INT UNSIGNED NOT NULL COMMENT 'プレイヤーID',
    CHARA_ID                 INT UNSIGNED NOT NULL COMMENT 'キャラクターID',
    REGISTER_DATETIME        DATETIME     NOT NULL COMMENT '登録日時',
    REGISTER_TRACE           VARCHAR(64)  NOT NULL COMMENT '登録トレース',
    UPDATE_DATETIME          DATETIME     NOT NULL COMMENT '更新日時',
    UPDATE_TRACE             VARCHAR(64)  NOT NULL COMMENT '更新トレース',
    PRIMARY KEY (PLAYER_FAVORITE_CHARA_ID),
    UNIQUE UQ_PLAYER_FAVORITE_CHARA (PLAYER_ID, CHARA_ID)
) COMMENT = 'お気に入りキャラ';
...
-- 本アプリにプレイヤー削除フローは無いため、他テーブルと同様 ON DELETE RESTRICT とする
-- (将来アカウント削除を追加する場合は、ログアウト同様にアプリ側で REFRESH_TOKEN を事前削除するか CASCADE を検討)
ALTER TABLE REFRESH_TOKEN
    ADD CONSTRAINT FK_REFRESH_TOKEN_PLAYER FOREIGN KEY (PLAYER_ID)
        REFERENCES PLAYER (PLAYER_ID)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT
;
...
ALTER TABLE ANALYZER_MEMO
    ADD CONSTRAINT FK_ANALYZER_MEMO_PLAYER FOREIGN KEY (PLAYER_ID)
        REFERENCES PLAYER (PLAYER_ID)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT
;
...
ALTER TABLE ANALYZER_MEMO
    ADD CONSTRAINT FK_ANALYZER_MEMO_VILLAGE FOREIGN KEY (VILLAGE_ID)
        REFERENCES VILLAGE (VILLAGE_ID)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT
;
...
ALTER TABLE PLAYER_FAVORITE_CHARA
    ADD CONSTRAINT FK_PLAYER_FAVORITE_CHARA_PLAYER FOREIGN KEY (PLAYER_ID)
        REFERENCES PLAYER (PLAYER_ID)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT
;
...
ALTER TABLE PLAYER_FAVORITE_CHARA
    ADD CONSTRAINT FK_PLAYER_FAVORITE_CHARA_CHARA FOREIGN KEY (CHARA_ID)
        REFERENCES CHARA (CHARA_ID)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT
;
