...
- **フェーズ**: **Step 3 (認証 REST 化) 完了 ✅**。Step 2 (monorepo 化) は完了済 (2.1 #47 / 2.2 #48 / 2.3 #49 / 2.4 e2e #50)
  ...
  - **次の候補**: (a) context-path rename `/wolf-mansion` → `/wolf-mansion-api` (別サブ step・下記)、または (b) **Step 4 (画面別 REST 化: ホーム→情報系→…)**。08-step-plan.md の順序に従う
...
**Step 3 (認証 REST 化) は 3.0〜3.4 で完了** 🎉。次は 08-step-plan.md の順序に従い、下記いずれかに着手する (どちらを先にするかは要判断)。

- **(候補A) context-path rename `/wolf-mansion` → `/wolf-mansion-api`** (別サブ step): `PlayerController.PATH_ID_REGISTER` の Cookie path 修正込み。稼働中 SSR と結合するため move と分離。**frontend は 3.3/3.4 で `VITE_API_BASE`/Vite proxy/gen:api の既定を `/wolf-mansion` 前提にしているため、rename 時に proxy キー・既定 base・cookie Path・`gen-api.mjs` の既定 base を整合させてまとめて更新する**
- **(候補B) Step 4 (画面別 REST 化)**: ホーム → 情報系 → ランダム → 新規村 → 村画面 → プロフィール の順 (08-step-plan.md)。`/add-issue` で起票 → `/ship-issue` で PR フロー (base = feature/monorepo)
