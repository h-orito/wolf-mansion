...
- **フェーズ**: **Step 4 (画面別 REST 化) 着手** — **step-4.1 (ホーム) 完了 ✅ (#56)** / **step-3.6 (認証画面 :8091 忠実再現) 完了 ✅ (#57)**。Step 3 (認証 REST 化) / Step 2 (monorepo 化) は完了済
  ...
  - **次の候補**: **step-4.2 (村一覧画面)** — 村一覧 API (`GET /api/v1/villages` status code 配列・4.1 で実装済) を共有し検索/絞り込み UI + 一覧表示を追加。`/add-issue` 起票 → `/ship-issue`。08-step-plan.md の順序に従う (認証画面忠実再現 step-3.6 は完了済、context-path rename は step-4.1 で完了済)
...
- **git 状態**:
  - ブランチ = `feature/monorepo`。HEAD = `cac2efef` (= step-3.6 #57)。作業ツリー clean、origin と同期
...
**Step 3 完了 🎉 + Step 4.1 (ホーム) 完了 ✅ (#56) + step-3.6 (認証画面忠実再現) 完了 ✅ (#57)**。context-path rename / 静的アセット移管も完了済。次は **step-4.2 (村一覧画面)** に着手する (08-step-plan.md の順序)。

- **(次) step-4.2 (村一覧画面)**: 村一覧 API (`GET /api/v1/villages` status code 配列・4.1 で実装済) を共有し、検索/絞り込み UI + 一覧表示を実装。一覧の行整形は既存 `features/villages/format.ts` を再利用 (必要なら行コンポーネントを home から `features/villages` へ昇格)。共通ヘッダー `components/layout/Header` (step-3.6 で作成済) を流用可。`/add-issue` 起票 → `/ship-issue` で PR フロー
