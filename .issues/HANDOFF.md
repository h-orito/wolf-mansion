...
- **フェーズ**: **Step 3 (認証 REST 化) 進行中**。Step 2 (monorepo 化) は完了済 (2.1 #47 / 2.2 #48 / 2.3 #49 / 2.4 e2e #50)
  ...
  - **次: step-3.4** OpenAPI→TS 型生成パイプライン (`pnpm gen:api` + CI drift 検知)。zod とポリシー定数の正式共有もここ
  - **UI 復元メモ (Step 12)**: 移行中は機能優先で各画面の `<title>` が仮 (例: 3.3 のログイン/トップ)。**画面ごとの title / OGP / `<head>` メタ / 共通ヘッダーは Step 12 (元画面の忠実復元) の明示スコープ**に追加済 (08-step-plan.md)。`:8091` 基準で復元する
...
- **git 状態**:
  - ブランチ = `feature/monorepo`。HEAD = `b0b92cd9` (= Step 3.3 #54)。作業ツリー clean、origin と同期
...
**step-3.4 (OpenAPI→TS 型生成)** に着手する。`/add-issue` で Issue 作成 → `/ship-issue step-3.4` で PR フロー (base = feature/monorepo)。

- **step-3.4 スコープ**: backend の OpenAPI スキーマから frontend の TS 型を生成するパイプライン (`pnpm gen:api` 等、devDep の `openapi-typescript` 導入済)。CI で drift 検知。**3.3 で暫定手書きした認証系の型 (`MeResponse`)・zod のパスワード/userId ポリシー定数を OpenAPI 経由の正式共有に置き換える**。3.3 の `app/lib/api.ts`/`features/auth/{api,schema}.ts` が移行対象
  - **3.3 の暫定箇所メモ**: `features/auth/api.ts` の `MeResponse` 手書き型、`features/auth/schema.ts` の `PASSWORD_MIN/MAX_LENGTH`・`PASSWORD_PATTERN`・userId 制約 (backend `PasswordPolicy`/`SignupRequest` と二重定義)。これらを生成型/共有定数に寄せる
- **別サブ step (Step 3 の後でも可)**: context-path rename `/wolf-mansion` → `/wolf-mansion-api` (`PlayerController.PATH_ID_REGISTER` の Cookie path 修正込み。稼働中 SSR と結合するため move と分離)。**frontend 側は 3.3 で `VITE_API_BASE`/Vite proxy のパスを `/wolf-mansion` 前提にしているため、rename 時に proxy キー・既定 base・cookie Path 整合をまとめて更新する**
- **UI 復元 (Step 12)**: 各画面の `<title>` / OGP / `<head>` メタ / 共通ヘッダーを `:8091` 基準で復元 (移行中は機能優先の仮 title)。08-step-plan.md の Step 12 スコープに明記済
