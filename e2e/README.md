...
**ローカル DB に登録済みの全ユーザーは password = `testuser`**。e2e でも手動の動作確認でも、既存ユーザーでログインする検証にそのまま使える
(signup で自己完結ユーザーを作る代わりに、既存プレイヤー前提のシナリオを書くときに便利)。

> これは **ローカル開発環境専用** のテストデータであり、本番には一切該当しない (公開しても問題ない情報)。
