...
public class BsFootstepCB extends AbstractConditionBean {
    ...
    //                                                                           Attribute
    //                                                                           =========
    protected FootstepCQ _conditionQuery;
     ...
     */
    public FootstepCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public FootstepCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected FootstepCQ doGetConditionQuery() {
    ...
    protected FootstepCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected FootstepCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        FootstepCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected FootstepCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new FootstepCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<FootstepCB> unionCBLambda) {
        final FootstepCB cb = new FootstepCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final FootstepCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<FootstepCB> unionCBLambda) {
        final FootstepCB cb = new FootstepCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final FootstepCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
     ...
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">footstep</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">footstep</span>.<span style="color: #CC4747">getVillageDay()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
     ...
     */
    public FootstepCB dreamCruiseCB() {
        FootstepCB cb = new FootstepCB();
        cb.xsetupForDreamCruise((FootstepCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<FootstepCB> columnQuery(final SpecifyQuery<FootstepCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected FootstepCB xcreateColumnQueryCB() {
        FootstepCB cb = new FootstepCB();
        cb.xsetupForColumnQuery((FootstepCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<FootstepCB> orCBLambda) {
        xorSQ((FootstepCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<FootstepCB> andCBLambda) {
        xorSQAP((FootstepCB)this, andCBLambda);
    }
    ...
    protected String xgetConditionBeanClassNameInternally() { return FootstepCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return FootstepCQ.class.getName(); }
