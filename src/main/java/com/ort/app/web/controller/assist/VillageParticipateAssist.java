...
public class VillageParticipateAssist {
    ...
    private void assertParticipate(Integer villageId, VillageParticipateForm participateForm) throws WolfMansionBusinessException {
        ...
        if (village.getVillageStatusCodeAsVillageStatus() != CDef.VillageStatus.募集中) {
            throw new WolfMansionBusinessException("プロローグが終了しているため入村できません。");
        }
        ...
        if (isAlreadyParticipateCharacter) {
            throw new WolfMansionBusinessException("既に参加されているキャラクターです。別のキャラクターを選択してください。");
        }
        if (BooleanUtils.isTrue(participateForm.getIsSpectator())) {
            ...
            if (charaNum - capacity <= spectatorNum) {
                throw new WolfMansionBusinessException("既に上限人数まで見学者がいるため見学者として参加できません。");
            }
        } else {
            ...
            if (village.getVillageSettingsAsOne().get().getPersonMaxNum() <= participateNum) {
                throw new WolfMansionBusinessException("既に上限人数まで参加しているプレイヤーがいるため参加できません。");
            }
            // 役職希望無効なのにおまかせ以外
            if ((!CDef.Skill.おまかせ.code().equals(participateForm.getRequestedSkill())
                ...
                throw new WolfMansionBusinessException("希望役職が不正です。");
            }
        }
    }
