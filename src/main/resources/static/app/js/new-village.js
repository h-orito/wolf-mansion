$(function() {
	...
	function replaceCharaSet(charaGroupId, manualChanged) {
		$.ajax({
			...
			// 発言欄
			replaceDummyChara(response[0].charaId, manualChanged);
			// プルダウン
	...
	function replaceDummyChara(charaId, manualChanged) {
		...
		if (dummyChara.joinMessage != null && dummyChara.joinMessage !== '') {
			if ($('#dummy-chara-join-message').val() === '') {
				$('#dummy-chara-join-message').val(dummyChara.joinMessage);
			} else {
