...
- なし
- ⚠️ **セキュリティ懸念**: 表示される名前は **ログイン ID そのもの** (`player.playerName` が認証 username 兼 表示名。別の表示名フィールドはなし)。この公開一覧で有効なログイン ID が列挙できる。**移行方針 (確定)**: ID と表示名の分離は行わず現状維持し、ブルートフォースはログイン側のレート制限で対応する ([auth-login.md](auth-login.md) / [03-auth.md](../03-auth.md) の「ログイン試行制限 / レート制限」)
