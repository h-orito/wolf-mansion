...
- **`/mypage` の統合 (step-3.6 で確定・ユーザー指示)**: step-3.6 で作った暫定 `/mypage` (最小情報 + パスワード変更導線 + ログアウト) を、本 step で `/user/{自分の名前}` と**同じ動作** (自分のプロフィール/戦績表示・本人なら自己紹介編集) に統合する。`/mypage` は `/user/{me.name}` への誘導 or プロフィールコンポーネント再利用に寄せる ([screens/player-profile.md](screens/player-profile.md) follow-up)
