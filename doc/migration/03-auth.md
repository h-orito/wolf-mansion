...
現状のログインは **試行回数が無制限**で、かつ **ユーザー一覧でログイン ID が列挙可能** (`player.playerName` が認証 username 兼 公開表示名。詳細は [auth-login.md](screens/auth-login.md) / [player-list.md](screens/player-list.md))。

- **ログイン ID と公開表示名の分離は行わない (現状維持)**。表示名カラム新設・一覧の非公開化等は本移行では実施しない
- ブルートフォース対策として **ログイン側にレート制限を導入** (確定方針):
  - **対象**: `POST /api/v1/auth/login` の失敗試行 (必要に応じ `POST /api/v1/auth/password` 等の資格情報検証にも適用)
  - **2 軸で制限**:
    - アカウント単位 (login ID): 一定時間窓で失敗 N 回超過したら以降を拒否
    - IP 単位: 同一 IP からの失敗総数を制限 (1 アカウント集中以外の横断的総当たりを抑止)
  - **超過時は `429 Too Many Requests`** を返す (UI は「しばらくしてから再試行」)
  - エラー応答は **ユーザー存在の有無を区別しない** (現状の「ユーザIDまたはパスワードが違います」を踏襲、列挙を助長しない)
  - **共有ストア必須**: backend は stateless + 複数インスタンス想定のため、試行カウンタはインメモリ不可。DB or Redis 等の共有ストアで管理 (Daychange のポーリング駆動と同じ multi-instance 論点)
- **補足**: パスワード下限 3 文字 (緩和後も維持) は短く、レート制限が実質的な唯一の総当たり防御になる。将来的な下限引き上げ・アカウントロック併用は検討余地 (現状は要望なし)
