...
- フロントエンドの**開発体験・技術スタックを現代化**する (Tailwind, TanStack Query, 型安全な API クライアント)
  - ⚠️ **見た目の UI/UX は現状を忠実に踏襲する** (本移行ではモダナイズしない)。各画面は移行する step 内で `:8091` 基準に忠実再現し、視覚的なモダナイズ (刷新) のみ cutover 後の別フェーズ (Step 12) で行う。詳細は [04-frontend.md](04-frontend.md) の「UI/UX 現状維持原則」
...
backend は Clean Architecture + DDD に基づいて設計されている。REST API もこの原則に従う。

- **画面専用の API やレスポンスを作らない**。API はドメインの操作・取得を提供するものであり、特定の画面レイアウトに合わせた整形・集約は行わない
- **可能な限りドメインモデルをそのまま返す**。`Charachip` のように隠すべき情報がないドメインオブジェクトは、専用の Response DTO を作らず直接シリアライズして返す
- **Response DTO を作ってよいケース**: (1) 認可マスクにより隠すべき情報がある場合、(2) 大量取得で不要な情報を削ぎ落とす場合（一覧 API 等）
- **複数のドメイン情報が必要な画面では、個別の API を叩いて frontend で組み立てる**。例: キャラチップ詳細の部屋割りは、キャラチップ取得 API と部屋割り情報取得 API を別々に呼び、frontend でグリッドを構築する
